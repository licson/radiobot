module.exports = function (bridge) {
	...
	function (config, makeLog, URLHandlers, queue, doQueueSong, songList, addToSongList, doTTS, mixer, volume) {
		...
		if (!/^\d+:[a-zA-Z0-9\-_]+$/.test(config.telegram.token)) {
			console.warn('[Telegram] [Error] Telegram front end failed to initialize because the auth token was missing.');
			return;
		}
		...
		function getFile(file) {
			return bot.getFile({ file_id: file }).then(function (data) {
				return config.telegram.apiServer + '/file/bot' + config.telegram.token + '/' + data.file_path;
			});
		}
		...
		bot._baseUrl = config.telegram.apiServer;
		bot.setMessageProvider(new Telegram.GetUpdateMessageProvider({ timeout: 60 }));
		bot.start();
		...
		handle(function (message) {
			...
			var title = message.audio.title || message.caption || null;
			...
			if (artist && title) {
				...
				log(`${new Date().toLocaleTimeString()} ${name}(${message.from.username}): [${title}] (${message.audio.file_id})`);
			} else {
				...
				getFile(message.document.file_id).then(function (url) {
					MediaInfo(url).then(function (info) {
						if (info.title && info.artist) {
							...
							ttsText = `Next is ${info.title} from ${name} on Telegram.`;
							titleText = `${info.title}`;
						} else if (message.caption) {
							titleText = `Song: ${message.caption}`;
						}
						...
						bot.sendMessage({
							chat_id: message.chat.id,
							reply_to_message_id: message.message_id,
							text: 'I don\'t understand your file format. Please try again with another file instead.'
						}).catch(logTGErrorMessage);
					});
				}).catch(function (e) {
					bot.sendMessage({
