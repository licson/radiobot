module.exports = function (bridge) {
	...
	function (config, makeLog, URLHandlers, queue, doQueueSong, songList, addToSongList, doTTS, mixer, volume) {
		...
		handle(/\/song_([1-9]\d*)(?:\s|@|$)/, function (message, match) {
			...
			var ttsText = `Next song is from ${songList[index].name}, picked up by ${message.from.username} on Telegram.`;
			var title = `Song picked up by ${message.from.username}.`;
		
			if (songList[index].title && songList[index].artist) {
				log(`${new Date().toLocaleTimeString()} ${message.from.username}(${message.from.username}): (from Song List) [${songList[index].artist} - ${songList[index].title}] (${songList[index].file})`);
				ttsText = `Next song is ${songList[index].title} performed by ${songList[index].artist} from ${songList[index].name}, picked up by ${message.from.username} on Telegram.`;
				title = `${songList[index].title} - ${songList[index].artist}`;
			}
		
			log(`${new Date().toLocaleTimeString()} ${message.from.username}(${message.from.username}): (${songList[index].file})`);
			
			var chatOrigin = ChatSource({
				type: 'telegram',
				generalName: `${message.from.username} on telegram`,
				chat_id: message.chat.id,
		...
		handle(function (message) {
			...
			var ttsText = `Next is ${message.document.file_name} from ${name} on Telegram.`;
			var titleText = `${message.document.file_name} from ${name}`;
