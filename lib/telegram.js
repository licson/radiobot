	...
	function (config, makeLog, URLHandlers, queue, doQueueSong, songList, addToSongList, doTTS, mixer, volume) {
		...
		function updatePlayList() {
			bot.editMessageText({
				...
				console.warn(`[Telegram Bot] failed to edit message due to ${err.toString()}, trying to resend new playlist...`);
				return bot.sendMessage({
					chat_id: config.telegram.playList,
					text: getPlayListText()
				}).then(function (message) {
				...
				console.error(`[Telegram Bot] failed to resend playlist due to ${err.toString()}`);
			});
		}
		...
		if (config.telegram.playList) {
			startPlayList().then(function () {
				if (queue.currentTask) {
					// enforce a update if queue updated before the listener was on.
					updatePlayList();
				}
				...
				console.warn(`[Telegram bot] failed to init play list due to ${err.toString()}`);
			});
		}
			...
			bridge.on('do_queue_song', function(chatOrigin, file, title, ttsText) {
				...
				if (originalChat.type === 'telegram') {
					bot.forwardMessage({
						...
						return bot.sendMessage({
							chat_id: config.telegram.logForward,
							text: `song ordered by ${chatOrigin.generalName}`,
							reply_to_message_id: res.message_id
						});
					}).catch(function (e) {
						console.warn('[Telegram Bot] Failed forwarding message! %s', e);
					});
				} else {
					bot.sendMessage({
						chat_id: config.telegram.logForward,
						...
						console.warn('[Telegram Bot] Failed sending message! %s', e);
					});
				}
			})
		}
