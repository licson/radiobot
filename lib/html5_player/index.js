module.exports = function (bridge) {
	bridge.allOnce(['get_router' ,'mp3_stream', 'queue', 'config', 'configureCORS', 'analytics'], function (getRouter, mp3Stream, queue, config, configureCORS, analytics) {
		const router = getRouter();
		const express = require('express');
		const albumArt = require('album-cover')(config.albumArt.key);
		...
		router.get('/status', configureCORS, function (req, res, next) {
			...
			res.end(`active_conn=${analytics.getTotalListeners()}
...
uptime=${process.uptime()}`);
		...
		router.get('/title/cover', function (req, res) {
			if (queue.currentTask != null) {
				...
				albumArt.search({
					artist: info[1] ? info[1].trim() : null,
					album: info[0] ? info[0].trim() : null,
					size: 'mega'
				}, function (e, photo) {
					if (e) {
						res.redirect('/components/default-artwork.png');
					} else if (photo == 'No image was found') {
						res.redirect('/components/default-artwork.png');
					} else {
						res.redirect(photo);
					}
