module.exports = function (bridge) {
	bridge.allOnce(['mixer', 'get_router', 'config', 'configureCORS'], function (distrib, getRouter, config, configureCORS) {
		...
		function doSegmentation(stream) {
			return new Promise(function (resolve, reject) {
				...
				var segmenter = spawn(require('ffmpeg-static').path, [
					'-f', 's16le',
		...
		router.get(/\/seg_(\d+)\.mp3/, function (req, res) {
			...
			var seg_path = path.join(tmpdir, 'seg_' + req.params[0] + '.mp3');
			var stream = fs.createReadStream(seg_path);
			stream.pipe(res);
		});
	});
};