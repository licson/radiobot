module.exports = function (bridge) {
	...
	function (config, makeLog, URLHandlers, queue, doQueueSong, songList, addToSongList, doTTS, mixer, volume) {
		...
		client.once('ready', function () {
			...
			// register the title event
			bridge.on('metadata', function(title) {
				client.user.setGame(title).catch(function (err) {
					console.error('[Discord Bot] Cannot set user status due to %s', err);
				});
			});
				...
				guild.channels.array().forEach(function (channel) {
					...
					if (guildConfig.audioChannel === channel.id && channel.type === "voice") {
						...
						channel.join().then(function (connection) {
							console.log(`[Discord bot] initing audio stream to ${channel.name} (${channel.id})`);
							
							// Workaround: It seems discord.js did something bad to the source stream and corrupt the internal state of the stream
							...
							dispatcher.on('end', function (reason) {
								console.log(`[Discord bot] stream to ${channel.name} (${channel.id}) was ended due to ${reason}`);
							});
							
							dispatcher.on('error', function (err) {
								console.log(`[Discord bot] error playing stream due to ${err}`);
							});
							
							connection.on('end', function () {
								console.log(`[Discord bot] connection to ${channel.name} (${channel.id}) was ended`);
							});
							
							connection.on('error', function (err) {
								console.log(`[Discord bot] error happened on ${channel.name} (${channel.id}): ${err}`);
							});
							
							connection.on('warn', function (warn) {
								console.log(`[Discord bot] warning playning stream: ${warn}`);
							});
		...
		handle(function(message) {
			...
			message.channel.fetchMessages({
				...
				if (res.size === 0 && config.discord.debug) {
					console.log(`[Discord bot] debug: current message is not the first message with ${ name } (${ userId })`);
				}
				if (res.size === 0) {
					message.author.sendMessage(config.discord.startMessage).catch(function (err) {
						console.warn(`[Discord bot] cannot send message to ${ name } ( ${ userId } ) due to ${ err.toString() }`);
					});
				}
			}).catch(function (err) {
				console.error('[Discord bot] cannot check whether current message is the first message due to %s', err);
			});
		});
		...
		handle(/^volume(\s|$)/, function (message) {
			...
			console.log(`[Discord Bot] New volume ${ newVolume } for type [${ temp[1] }] set by ${ name }`);
		...
		bridge.on('text_message', function (chatOrigin, text) {
			...
			client.fetchUser(chatOrigin.user_id).then(function (user) {
				...
				console.warn(`[Discord bot] cannot send message to ${ chatOrigin.user_id } due to ${ err.toString() }`);
			});
		});
