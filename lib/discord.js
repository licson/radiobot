	...
	function (config, makeLog, URLHandlers, queue, doQueueSong, songList, addToSongList, doTTS, mixer, volume) {
		...
		function startAudioAndLog(clinet, config) {
				...
				guild.channels.array().forEach(function (channel) {
					...
					if (guildConfig.audioChannel === channel.id && channel.type === "voice") {
						...
						channel.join().then(function (connection) {
							console.log(`[Discord] initing audio stream to ${channel.name} (${channel.id})`);
							...
							dispatcher.on('end', function (reason) {
								console.log(`[Discord] stream to ${channel.name} (${channel.id}) was ended due to ${reason}`);
							});
							
							dispatcher.on('error', function (err) {
								console.log(`[Discord] error playing stream due to ${err}`);
							});
							
							connection.on('end', function () {
								console.log(`[Discord] connection to ${channel.name} (${channel.id}) was ended`);
							});
							
							connection.on('error', function (err) {
								console.log(`[Discord] error happened on ${channel.name} (${channel.id}): ${err}`);
							});
							
							connection.on('warn', function (warn) {
								console.log(`[Discord] warning playning stream: ${warn}`);
							});
			...
			startPlayList().then(function () {
				...
				console.warn(`[Discord] error during initing the play list: ${err.toString()}`);
			});;
		}
		...
		function updatePlayList() {
			Promise.all(playLists.map(function (playList) {
				...
				console.warn(`[Discord] error during update the play list: ${err.toString()}`);
			});
		}
		...
		handle(function(message) {
			...
			message.channel.fetchMessages({
				...
				if (res.size === 0 && config.discord.debug) {
					console.log(`[Discord] debug: current message is not the first message with ${ name } (${ userId })`);
				}
		...
		handle(/^list(\s|$)/, function (message) {
			...
			message.author.sendMessage(output).catch(function (err) {
				console.warn(`[Discord] cannot send message to ${ name } ( ${ userId } ) due to ${ err.toString() }`);
			});
		});
		...
		handle(/^volume(\s|$)/, function (message) {
			...
			console.log(`[Discord] New volume ${ newVolume } for type [${ temp[1] }] set by ${ name }`);
		...
		bridge.on('text_message', function (chatOrigin, text) {
			...
			client.fetchUser(chatOrigin.user_id).then(function (user) {
				...
				console.warn(`[Discord] cannot send message to ${ chatOrigin.user_id } due to ${ err.toString() }`);
			});
		});
