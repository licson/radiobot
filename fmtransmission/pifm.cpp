/*
 * Originally created by Oliver Mattos and Oskar Weigl, modified by Licson to
 * accept streaming input and present as a node.js native addon. 
 * 
...
namespace FMTransmission {
	...
	class Outputter : public SampleSink
	{
		...
		Outputter(float rate) : sleeptime((float)1e6 * BUFFERINSTRUCTIONS / 4 / rate / 2), // sleep time is half of the time to empty the buffer
								fracerror(0),
								timeErr(0)
		{
	...
	class PreEmp : public SampleSink
	{
		...
		PreEmp(float rate, SampleSink *next) : fmconstant(rate * 75.0e-6), // for pre-emphisis filter.  75us time constant
											dataold(0),
											next(next){};
	...
	class Resamp : public SampleSink
	{
		...
		Resamp(float rateIn, float rateOut, SampleSink *next) : outTimeLeft(1.0),
																outBufPtr(0),
	...
	class StereoSplitter : public SampleSink
	{
		...
		StereoSplitter(SampleSink *nextLeft, SampleSink *nextRight) : nextLeft(nextLeft), nextRight(nextRight) {}
	...
	class RDSEncoder : public SampleSink
	{
		...
		RDSEncoder(SampleSink *next) : next(next), bitNum(0), lastBit(0), time(0), lastValue(0)
		{
			for (int i = 0; i < 8; i++)
			{
	...
	class StereoModulator : public SampleSink
	{
		public:
			...
			class ModulatorInput : public SampleSink
			{
				...
				ModulatorInput(StereoModulator *mod, int channel) : mod(mod),
																	channel(channel) {}
		...
		StereoModulator(SampleSink *next) : next(next), bufferOwner(0), bufferLen(0), state(0)
		{
			for (int i = 0; i < 16; i++)
			{
	...
	void Init(const FunctionCallbackInfo<Value> &args)
	{
		if (args.Length() < 3)
		{
			Nan::ThrowError("Usage: init(samplerate, channels, broadcastFreq)");
			return;
		}

		sampleRate = args[0]->Uint32Value();
		channels = args[1]->Uint32Value();
		fmFreq = args[2]->NumberValue();

		// Setup DMA
		setupDMA((float)fmFreq);

		if (channels == 2)
		{
			StereoModulator* sm = new StereoModulator(new RDSEncoder(new Outputter(152000)));
			pctx = new StereoSplitter( 
		...
		{
			pctx = new Mono(new PreEmp(sampleRate, new Outputter(sampleRate)));
		}
	}
	...
	void Write(const FunctionCallbackInfo<Value> &args)
	{
		if (args.Length() != 1)
		{
			Nan::ThrowError("No data to write!");
			return;
		}

		if (!args[0]->IsObject()) {
			Nan::ThrowTypeError("The data is not a buffer object!");
			return;
		}
		...
		if (pctx != nullptr)
		{
			pctx->consume(buffer, strlen(buffer));
		}
		else {
			Nan::ThrowError("FM transmission probably not ready. Please call init() first.");
			return;
		}
	}

	void ModuleInit(Local<Object> exports, Local<Object> module)
	{
		Nan::SetMethod(exports, "init", Init);
		Nan::SetMethod(exports, "destroy", Destroy);
		Nan::SetMethod(exports, "write", Write);
	}

	NODE_MODULE(fm, ModuleInit);
}